...
namespace Runtime
{
		...
		m_vulkan_context{ std::move(vulkan_context) }
	{
		...
		// Palette
		Palette::SET0_UBO_Layout = m_vulkan_context->CreateDescripotrSetLayout
		({
		{Palette::binding_camera_matrics, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT},
		{Palette::binding_light_parameters, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT}
		});

		Palette::SET1_Materials_Layout = m_vulkan_context->CreateDescripotrSetLayout
		({
